	...
	try {
		...
		if (emailExists) {
			res.status(409).json({
				status: 409,
				data: req.body,
				message: "Email matches existing user.",
			});
		} else if (!results) {
			res.status(404).json({ status: 404, message: "Please try again." });
		} else if (firstNameTest || lastNameTest || password.length < 6) {
			res.status(400).json({
				status: 400,
				data: req.body,
				message:
					"Invalid entry on form. Please try again. Note that password must be more than six characters.",
			});
		} else if (!isEmail) {
			res.status(400).json({
				status: 400,
			...
			let newUser = {
				...
				bio: "Add a bio to your profile in settings",
				bannerSrc: "",
				_id: id,
				movieReviews: [],
			};
